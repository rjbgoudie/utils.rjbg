...
#' 
#' Suppose we have a data frame \code{x}, composed of a number of factor variables 
#' (in columns).
#' These factor variables are poorly named. For example, they may just be named by 
#' by integers. We would like to rename them so that their names are more helpful and 
#' intuitive.
#'
#' To do this in a clear and repeatable manner, we will create an R list. Each 
#' component of the list is given the name of a column of the data.frame \code{x}.
#' Each of these components is a character vector, each component of which 
#' refers to a level of the corresponding factor variable in \code{x}. 
#' Each of these component of the vector is a character, taking the default 
#' value of the current name of the factor level.
#' 
#' The function \code{\link{generateFactorRenamer}} can be used to create such a list.
#' This can then be altered, and then processed using this function.
#' 
#' @param x a data frame
#' @param levelChanges A list, of the form generated by 
#'   \code{\link{generateFactorRenamer}}
#' @param allowMissingCols A logical or a character vector. If it is a logical, it 
#'   indicates whether to allow some columns of x 
#'   not to be included in levelChanges. If TRUE, only a warning is issue if there are 
#'   columns missing. If FALSE, an error is given.
#'   If it is a character vector, it contains the names of any columns that are allowed
#'   not to be included in levelChanges.
#' @param verbose A logical, indicating whether to show progress
#' @return The data frame \code{x}, with factor levels releveled as prescribed by 
#'   \code{levelChanges}.
#' @examples
#' dat <- esoph[, 1:3]
...
#' 
#' Creates a template from which a specification of the renaming of the levels of 
#' factor variables in a data.frame can be.
#'
#' Suppose we have a data frame \code{x}, composed of a number of factor variables 
#' (in columns).
#' These factor variables are poorly named. For example, they may just be named by 
...
#' The list can then be modified to specify the new level names, and the changes 
...
#' 
#' Relevels a factor column of a data frame, as as prescribed by a level changes 
#' list, of the form generated by \code{\link{generateFactorRenamer}}.
#' 
#' @param colName A character vector of length 1, indicating which column of \code{x}
#'   the string should be created for.
#' @param x a data frame
#' @param levelChanges A list, of the form generated by 
#'   \code{\link{generateFactorRenamer}}
#' @param verbose A logical, indicating whether to show progress
...
#'   the string should be created for.
#' @param x a data frame
#' @return A character vector. This is an individual component of the list described 
#'   in \code{\link{generateFactorRenamer}}.
levelsStringForColName <- function(colName, x){
  ...
  quotedLevels <- sapply(colLevels, function(q) paste("\"", q, "\"", sep = ""))
  allQuotedLevels <- paste(quotedLevels, " = ", quotedLevels, collapse = ", ", sep = "")
